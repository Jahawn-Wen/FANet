...
def make_dataset_sues(dir, class_to_idx, extensions):
    images = []
    dir = os.path.expanduser(dir)
    for target in sorted(os.listdir(dir)):
        d = os.path.join(dir, target)
        ...
        # Iterate over first-level subfolders, such as drone/150 and drone/200.
        for subfolder in sorted(os.listdir(d)):
            subfolder_path = os.path.join(d, subfolder)
            if os.path.isdir(subfolder_path):
                # Collect images inside second-level subfolders, such as drone/150.
                for root, _, fnames in sorted(os.walk(subfolder_path)):
                    for fname in sorted(fnames):
                        if has_file_allowed_extension(fname, extensions):
                            path = os.path.join(root, fname)
                            item = (path, class_to_idx[target])
                ...
                # Handle single-image entries, such as satellite/0000001.
                if has_file_allowed_extension(subfolder, extensions):
                    path = subfolder_path
                    item = (path, class_to_idx[target])
                    images.append(item)
...
class customDataSUES(Data.Dataset):
    def __init__(self, root, transform=None, target_transform=None, loader=default_loader, rotate=0, pad=0):
        classes, class_to_idx = find_classes(root)
        IMG_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.ppm', '.bmp', '.pgm', '.tif', '.webp']
        imgs = make_dataset_sues(root, class_to_idx, IMG_EXTENSIONS)
        if len(imgs) == 0:
            raise(RuntimeError("Found 0 images in subfolders of: " + root + "\n"
                               "Supported image extensions are: " + ",".join(IMG_EXTENSIONS)))
    ...
    def __getitem__(self, index):
        ...
        Returns:tuple: (image, target) where target is class_index of the target class.
            ...
            if self.iaa_trans is not None:
                ...
                # img_aug.save('/tmp/%s'%path.split('/')[-1])
            # else:
            #     img = np.array(img)
            #     # img = Image.fromarray(img)
            if self.transform is not None:
    ...
    def __getitem__(self, index):
        ...
        Returns:tuple: (image, target) where target is class_index of the target class.
            ...
            if self.iaa_trans is not None:
                ...
                # img_aug.save('/tmp/test_img/%s'%path.split('/')[-1])
            if self.transform is not None:
                img = self.transform(img)
            if self.target_transform is not None:
                target = self.target_transform(target)
