...
import numpy as np  
from segment.segment_anything import SamPredictor, sam_model_registry, SamAutomaticMaskGenerator
...
# TODO 
class WeatherWeightNet(nn.Module):
    def __init__(self, num_conditions):
        super(WeatherWeightNet, self).__init__()
        self.network = nn.Sequential(
            nn.Linear(num_conditions, 64),
            nn.ReLU(),
            nn.Linear(64, 1)
        )

    def forward(self, x):
        return torch.sigmoid(self.network(x))  # Use sigmoid to keep weights positive.

def compute_weighted_loss(predictions, targets, weights):
    criterion = nn.CrossEntropyLoss(reduction='none')
    losses = criterion(predictions, targets)
    weighted_losses = losses * weights.squeeze()  # Ensure weights broadcast correctly.
...
class two_view_net(nn.Module):
    def __init__(self, class_num, droprate, stride = 1, pool = 'avg', share_weight = False, VGG16=False, LPN=False, block=2, norm='bn', adain='a', btnk=[1,0,1]):
        ...
        self.norm = norm
        self.adain = adain
        self.sqr = False # if the satellite image is square ring partition and the ground image is row partition, self.sqr is True. Otherwise it is False.
        if VGG16:
            if LPN:
                # satelite
        ...
        if LPN:
            ...
            if VGG16:
                ...
                # self.classifier = ClassBlock(4096, class_num, droprate, num_bottleneck=512)  # SAFA setting.
                if pool =='avg+max':
...
class ResNetRPP(nn.Module):
    def __init__(self, num_classes, num_parts=4, pretrained=True):
        super(ResNetRPP, self).__init__()
        # Load a pre-trained ResNet
        self.base = models.resnet50(pretrained=pretrained)
        
        # Modify the last layer for the specific number of classes
        self.base.fc = nn.Linear(self.base.fc.in_features, num_classes)
        
        # Adding the RPP layers as described in the original code
        self.num_parts = num_parts
        self.dropout = nn.Dropout(0.5)
        self.local_conv = nn.Conv2d(2048, self.base.fc.in_features, kernel_size=1, padding=0, bias=False)
        nn.init.kaiming_normal_(self.local_conv.weight, mode='fan_out')
        self.local_mask = nn.Conv2d(self.base.fc.in_features, num_parts, kernel_size=1, padding=0, bias=True)
        nn.init.kaiming_normal_(self.local_mask.weight, mode='fan_out')
        nn.init.constant_(self.local_mask.bias, 0)
        self.FC = nn.Linear(8192, 2048)

    def forward(self, x):
        # Standard ResNet processing
        x = self.base.conv1(x)
        x = self.base.bn1(x)
        x = self.base.relu(x)
        ...
        x = self.local_conv(x)
        x = F.relu(x)
        mask = self.local_mask(x)
        mask = F.softmax(mask, dim=1)
        
        parts = []
        for i in range(self.num_parts):
            part = F.adaptive_avg_pool2d(x * mask[:, i:i+1, :, :], (1, 1))
            parts.append(part.view(part.size(0), -1))
            # print(part.shape)
        out = torch.cat(parts, dim=1)
        # print("//")
        # print(out.shape)
        out = self.FC(out)
        # out = self.base.fc(out)
        return out
...
class three_view_net(nn.Module):
    def __init__(self, class_num, droprate, stride = 2, pool = 'avg', share_weight = False, VGG16=False, LPN=False, block=6, norm='bn', adain='a', circle=False, btnk=[1,0,1], conv_norm='none', Dense=False, ResNet101=False):
        ...
        self.adain = adain
        self.circle = circle
        self.rpp = ResNetRPP(num_classes=class_num, num_parts=4, pretrained=True)
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.sam_predictor = self.initialize_sam()
        self.register_parameter('rpp_channel_weights', None)
        self.attention = nn.Sequential(
            nn.Conv2d(6, 64, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(64, 2, kernel_size=1),  # Output 2 channels
            nn.Softmax(dim=1)
        )
    ...
    def initialize_sam(self):
        checkpoint_path = os.environ.get(
            "SAM_CHECKPOINT",
            os.path.join(os.path.dirname(__file__), "segment", "checkpoint", "sam_vit_h_4b8939.pth"),
        )
        sam = sam_model_registry["vit_h"](checkpoint=checkpoint_path).to(self.device)
        ...
        if self.LPN:
            ...
            if x1 is None:
                ...
                if self.norm == 'ada-ibn':
                    ...
                    x6 = self.model_1(x1, smod_f)
                    
                    x1_rpp = self.rpp(x1)
                    x1_rpp_weights = torch.sigmoid(x1_rpp)

                    masks, scores = self.get_sam_masks(x1[0].cpu().numpy().transpose(1, 2, 0))
                    max_score_index = scores.argmax()
                    highest_score_mask = masks[max_score_index]

                    highest_score_mask = torch.tensor(highest_score_mask, device=x1.device, dtype=torch.float32)
                    highest_score_mask = F.interpolate(highest_score_mask.unsqueeze(0).unsqueeze(0), size=x1.shape[2:], mode='nearest')
                    highest_score_mask = highest_score_mask.expand(x1.size(0), 1, x1.size(2), x1.size(3))
    

                    # Compute attention weights.
                    attention_input = torch.cat([x1, x1 * highest_score_mask], dim=1)
                    attention_weights = self.attention(attention_input)

                    # Apply attention.
                    x1_weighted = x1 * attention_weights[:, 0:1, :, :]
                    x1_sam_weighted = x1 * highest_score_mask * attention_weights[:, 1:2, :, :]

                    # Combine features.
                    x1_combined = x1_weighted + x1_sam_weighted

                    # Forward through the model.
                    x1 = self.model_1(x1_combined, smod_f)

                    alpha = 0.3  # Weight coefficient.
                    beta = 0.3  # Balance coefficient.
                    x1 = (1 - alpha) * x6 + alpha * (beta * x1 + (1 - beta) * x6 * x1_rpp_weights)
            ...
            if x3 is None:
                ...
                if self.norm == 'ada-ibn':
                    ...
                    dmod_f, dout_w = self.pt_model(x3)
                    x5 = self.model_3(x3, dmod_f)
                    x3_rpp = self.rpp(x3)
                    x3_rpp_weights = torch.sigmoid(x3_rpp)

                    masks, scores = self.get_sam_masks(x3[0].cpu().numpy().transpose(1, 2, 0))
                    max_score_index = scores.argmax()
                    highest_score_mask = masks[max_score_index]

                    highest_score_mask = torch.tensor(highest_score_mask, device=x3.device, dtype=torch.float32)
                    highest_score_mask = F.interpolate(highest_score_mask.unsqueeze(0).unsqueeze(0), size=x3.shape[2:], mode='bilinear', align_corners=False)
                    highest_score_mask = highest_score_mask.expand(x3.size(0), 1, x3.size(2), x3.size(3))

                    if not hasattr(self, 'attention'):
                        self.attention = nn.Sequential(
                            nn.Conv2d(x3.size(1) * 2, 64, kernel_size=3, padding=1),
                            nn.ReLU(),
                            nn.Conv2d(64, 2, kernel_size=1),  # Output two channels.
                            nn.Softmax(dim=1)
                        ).to(x3.device)

                    # Compute attention weights.
                    attention_input = torch.cat([x3, x3 * highest_score_mask], dim=1)
                    attention_weights = self.attention(attention_input)

                    # Apply attention.
                    x3_weighted = x3 * attention_weights[:, 0:1, :, :]
                    x3_sam_weighted = x3 * highest_score_mask * attention_weights[:, 1:2, :, :]

                    # Combine features.
                    x3_combined = x3_weighted + x3_sam_weighted

                    # Forward through the model.
                    x3 = self.model_3(x3_combined, dmod_f)

                    alpha = 0.3  # Weight coefficient.
                    beta = 0.3  # Balance coefficient.
                    x3 = (1 - alpha) * x5 + alpha * (beta * x3 + (1 - beta) * x5 * x3_rpp_weights)
