...
def load_network(name, opt):
    ...
    epoch = epoch.split('.')[0]
    if epoch != 'last' and epoch.isdigit():
       epoch = int(epoch)
    config_path = os.path.join(dirname,'opts.yaml')
    with open(config_path, 'r') as stream:
        config = yaml.load(stream, Loader=yaml.FullLoader)

    # Which model implementation to use when re-loading the checkpoint.
    # Default keeps backward compatibility with existing runs.
    opt.model_impl = config.get('model_impl', 'model')
    ...
    # Optional: fusion weights used by some model variants (e.g., model_zsycs).
    opt.fusion_alpha = config.get('fusion_alpha', 0.3)
    opt.fusion_beta = config.get('fusion_beta', 0.3)
    ...
    # Defer model import until we know which implementation to use.
    model_impl = str(opt.model_impl).lower()
    if model_impl in {'zsycs', 'model_zsycs'}:
        from model_zsycs import ft_net, two_view_net, three_view_net  # type: ignore
        _use_fusion_kwargs = True
    elif model_impl in {'loss', 'model_loss'}:
        from model_loss import ft_net, two_view_net, three_view_net  # type: ignore
        _use_fusion_kwargs = False
    else:
        from model import ft_net, two_view_net, three_view_net  # type: ignore
        _use_fusion_kwargs = False

    if opt.views == 3:
        if opt.LPN:
            if _use_fusion_kwargs:
            ...
            if 'use_res101' in config and opt.use_res101:
                ...
                # model = three_view_net(opt.nclasses, opt.droprate, stride = opt.stride, pool = opt.pool, share_weight = opt.share, norm = opt.norm, adain = opt.adain, btnk=opt.btnk)
